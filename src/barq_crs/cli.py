...
def _emit_jsonl(values: Any) -> None:
    for value in values:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="barq",
        description="Evidence-gated verification and reasoning for authorized security research",
    )
    commands = parser.add_subparsers(dest="command", required=True)

    authz = commands.add_parser(
        "authz", help="analyze controlled-identity response observations"
    )
    ...
    variants = commands.add_parser(
        "variants", help="mine source siblings from a security patch"
    )
    variants.add_argument("diff")
    variants.add_argument("root")

    race = commands.add_parser(
        "collisions", help="model candidate state-transition collisions"
    )
    race.add_argument("transitions")

    triage = commands.add_parser(
        "ctf-triage", help="classify an isolated challenge directory"
    )
    triage.add_argument("root")

    flag = commands.add_parser(
        "flag-check", help="apply a local flag oracle to a text file"
    )
    ...
    active_verify = commands.add_parser(
        "verify",
        help=(
            "actively verify read-only authorization boundaries with explicitly "
            "controlled accounts and resources"
        ),
    )
    ...
    rank.add_argument("--campaign", default="authorized-research")

    sarif = commands.add_parser("sarif", help="convert BARQ candidates to SARIF 2.1.0")
    sarif.add_argument("candidates")
    sarif.add_argument("output")

    ledger_verify = commands.add_parser(
        "ledger-verify", help="verify an evidence ledger hash chain"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "authz":
        findings = AuthorizationDifferentialEngine().analyze(
            Observation.from_dict(value) for value in _jsonl(args.observations)
        )
        ...
        findings = PatchSeededVariantEngine().analyze(
            Path(args.diff).read_text(encoding="utf-8"), args.root
        )
        _emit([item.to_dict() for item in findings])
    elif args.command == "collisions":
        findings = StateCollisionEngine().analyze(
            StateTransition.from_dict(value) for value in _json(args.transitions)
        )
        _emit([item.to_dict() for item in findings])
        ...
        ranked = SignalFusion().rank(_candidate(value) for value in _json(args.candidates))
        if args.report:
            Path(args.report).write_text(
                markdown_report(ranked, args.campaign), encoding="utf-8"
            )
        _emit([item.to_dict() for item in ranked])
    elif args.command == "sarif":
        value = sarif_report(_candidate(item) for item in _json(args.candidates))
