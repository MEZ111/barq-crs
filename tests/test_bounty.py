...
def test_bounty_runner_builds_ranked_board_and_delta(tmp_path: Path) -> None:
    _policy(tmp_path / "policy.json")
    ...
    _write_json(
        tmp_path / "campaign.json",
        {
            ...
            "max_leads": 20,
            "recon": {"env": {"NUCLEI_RPS": "3", "KATANA_RPS": "2"}},
        },
    )
    ...
    output = tmp_path / "out"
    first = BountyRunner(command_runner=fake_runner).run(tmp_path / "campaign.json", output)
    assert first.lead_count == 3
    assert first.scanner_signal_count == 1
    assert first.new_lead_count == 3
    board = json.loads((output / "bounty-board.json").read_text(encoding="utf-8"))
    assert board[0]["confidence"] == "scanner-signal"
    ...
    second = BountyRunner(command_runner=fake_runner).run(tmp_path / "campaign.json", output)
...
def test_bounty_runner_rejects_domain_outside_policy(tmp_path: Path) -> None:
    ...
    with pytest.raises(BountyError, match="not explicitly covered"):
        BountyRunner(command_runner=lambda *_: None).run(
            tmp_path / "campaign.json", tmp_path / "out"
        )


def test_bounty_mode_caps_rate_overrides() -> None:
    with pytest.raises(BountyError, match="cannot exceed 20"):
        BountyRunner._bounded_env({"env": {"NUCLEI_RPS": "21"}})
